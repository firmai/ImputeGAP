...
class TestPRISTI(unittest.TestCase):
    ...
    def test_imputation_pristi_udef(self):
        ...
        """
        ts_1 = TimeSeries()
        ts_1.load_series(utils.search_path("eeg-alcohol"), nbr_series=40)
        ts_1.normalize(normalizer="min_max")

        incomp_data = ts_1.Contamination.mcar(input_data=ts_1.data)

        algo = Imputation.DeepLearning.PRISTI(incomp_data).impute(params={"target_strategy":"hybrid", "unconditional":True, "batch_size":10, "embedding":12, "seed": 42})
        algo.score(ts_1.data)
        metrics = algo.metrics
