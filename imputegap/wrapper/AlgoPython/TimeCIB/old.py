...
from imputegap.wrapper.AlgoPython.TimeCIB.models import VAE, HI_VAE, GP_VAE, TimeCIB, GaussianDecoder
from imputegap.wrapper.AlgoPython.TimeCIB.models import DiagonalEncoder, JointEncoder, BandedJointEncoder, RNNEncoder
from imputegap.wrapper.AlgoPython.TimeCIB.models import ImagePreprocessor
from imputegap.wrapper.AlgoPython.TimeCIB.models.util_get_scores import get_imputed_inputs, get_discriminative_score, get_predictive_score
...
def recoveryTimeCIB(miss_np_data):
    ...
    """
    # Replace argparse with SimpleNamespace for default arguments
    args = SimpleNamespace(
        ...
        normalize=True,
        cont_sigma=1.0,
        batch_size=miss_np_data.shape[0],  # Set batch size dynamically
        data_dim=miss_np_data.shape[1],  # Set data dimension dynamically
        binary=False,
        time_length=miss_np_data.shape[1],
        num_classes=0,
        image_shape=None,
    ...
    # If dataset requires an image preprocessor, initialize it
    if args.dataset in ["hmnist", "rotated"]:
        args.image_preprocessor = ImagePreprocessor(args)
    ...
    train_data = torch.tensor(miss_np_data, dtype=torch.float32)
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, shuffle=True)

    # Initialize TimeCIB model
    model = TimeCIB(args)
    model.to(device)
    ...
    for epoch in range(args.num_epoch):
        ...
        for batch in tqdm(train_loader):
            x_miss = batch.to(device)
            loss, *_ = model(x_miss, x_miss, torch.isnan(x_miss).float(), torch.isnan(x_miss).float(), None)
    ...
    model.eval()
    x_imputed = model.sample(train_data.to(device), None, torch.isnan(train_data).to(device))
    x_imputed = x_imputed.detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    miss_data = ts_1.Contamination.missing_percentage(ts_1.data, rate_series=0.4)
