...
class DataLoader(object):
    ...
    def generate_mask_nqu(self, input):
        """
        Generates a mask based on NaN values in the numpy matrix.
        Elements corresponding to NaN values are set to 0, others to 1.
        """
        if not isinstance(self._data, np.ndarray):
            logging.error("Data must be a numpy array")
            return

        # Initialize the mask as all ones
        self._mask = np.ones((self._data.shape[1],), dtype=np.int8)

        # Identify columns with NaN values and set corresponding mask elements to 0
        nan_indices = np.where(np.isnan(input).any(axis=1))[0]

        self._mask[nan_indices] = 0
