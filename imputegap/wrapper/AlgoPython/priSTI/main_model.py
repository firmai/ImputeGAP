...
class PriSTI(nn.Module):
    def __init__(self, train_loader, target_dim, seq_len, config, device):
        super().__init__()
        self.device = device
        ...
        input_dim = 2
        self.diffmodel = Guide_diff(train_loader, config_diff, input_dim, target_dim, self.use_guide)
...
class PriSTI_aqi36(PriSTI):
    def __init__(self, config, device, target_dim=16, seq_len=16):
...
class PriSTI_PemsBAY(PriSTI):
    def __init__(self, train_loader, config, device, target_dim=256, seq_len=24):
        super(PriSTI_PemsBAY, self).__init__(train_loader, target_dim, seq_len, config, device)
        self.config = config
