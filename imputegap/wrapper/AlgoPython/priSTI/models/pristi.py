...
from imputegap.wrapper.AlgoPython.priSTI.models.diffusion import Guide_diff
...
class PriSTI(nn.Module):
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples, itp_info):
        ...
        for i in range(n_samples):
            ...
            imputed_samples = current_sample.detach().permute(0,2, 1).squeeze(0)
            # print(current_sample.detach().size())
...
class PriSTI_(PriSTI):
    def __init__(self, config, device, target_dim, seq_len):
        super(PriSTI_, self).__init__(target_dim, seq_len, config, device)
        self.config = config

    def process_data(self, batch):
        _, n, dim = batch["observed_data"].shape
        observed_data = batch["observed_data"].to(self.device).float()
        observed_mask = batch["observed_mask"].to(self.device).float()
        observed_tp = batch["timepoints"].reshape((1, -1)).to(self.device).float()
        gt_mask = batch["observed_data"].to(self.device).float()
        cut_length = [0]*n
        ...
        observed_data = observed_data.reshape((1, n, dim)).permute(0,2,1)  # [B, K, L]
        # observed_mask = observed_mask.permute(0, 2, 1)
        observed_mask = observed_mask.reshape((1, n, dim)).permute(0,2,1)
        gt_mask = gt_mask.reshape((1, n, dim)).permute(0, 2, 1)
        ...
        if self.config['model']['use_guide']:
            coeffs = coeffs.reshape((1, n, dim)).permute(0, 2, 1)
