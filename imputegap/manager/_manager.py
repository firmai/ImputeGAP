...
class TimeSeriesGAP:
    ...
    def __init__(self, filename):
        ...
        :param contaminated_ts : time series after contimination
    ...
    def print_results(self):
        ...
        print("RMSE          (root mean square error)         = ", self.imputation_metrics[0])
        print("MAE           (mean absolute error)            = ", self.imputation_metrics[1])
        print("MI            (mutual information discrete)    = ", self.imputation_metrics[2])
        print("CORRELATION   (pearson correlation)            = ", self.imputation_metrics[3], "\n\n")
    ...
    def plot(self, ts_type="ground_truth", title='Time Series Data', save_path="", limitation=10, size=(16,8), display=True, colors = ['dimgrey', 'plum', 'lightblue', 'mediumseegreen', 'khaki']):
        ...
        if ts_type == "ground_truth":
            ...
            for i in range(self.ts.shape[0]):
                ...
                #plt.plot(np.arange(self.ts.shape[1]), self.ts[i, :], 'r--',label=f'Series {i + 1}-GT')
                #plt.plot(np.arange(self.contaminated_ts.shape[1]), self.contaminated_ts[i, :], linewidth=2.5, color=color, linestyle='-', label=f'Series {i + 1}-MV')
                plt.plot(np.arange(self.imputation.shape[1]), self.imputation[i, :], linewidth=2.5, color=color, linestyle='-', label=f'Series{i + 1}-IMP')
    ...
    def contamination_mcar(self, ts = None, missing_rate=0.1, block_size=10, starting_position=0.1, series_selected=["*"], use_seed=True, seed=42):
        """
        ...
        self.contaminated_ts = ContaminationGAP(self).contamination_mcar(ts, missing_rate, block_size, starting_position, series_selected, use_seed, seed)
